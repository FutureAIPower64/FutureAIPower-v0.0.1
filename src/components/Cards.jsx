...
function Cards() {
    ...
    console.log(cards);
    // var carddata=useSelector((state)=>state.counter.carddata)
    // var singlecate=useSelector((state)=>state.counter.singlecate)
    // console.log(cards);
    ...
    //         cards=cards;
    //     }else{
    //         cards=cards.filter((ele)=>{
    //             return ele.cate==singlecate;
    //         })
    //     }
    // }
    // useEffect(()=>{
    ...
    //         } else {
    //             setFilteredCards(cards.filter(ele => ele.cate === singlecate));
    //         }
    //     }, [singlecate, cards]);
    ...
    //     console.log(singlecate);
    //     useEffect(() => {
    //         alert(singlecate); // Alert whenever singlecate changes
    //     }, [singlecate]);
            ...
            <div className='w-10/12 px-4'>
                <div className='flex  justify-between'>
                    <OwlCarousel className='owl-theme' autoplay={true} autoplayTimeout={3000} margin={12} loop dots={false} responsive={{ 0: { items: 1 }, 600: { items: 3 }, 1000: { items: 4 } }}>
                        {
                            cards.map((card, ind) => (
                                // card.cate===singlecate?
                                <div key={ind} className=' border-2 rounded-lg px-4 py-3  bg-slate-200 w-full h-72'>
